...
@php
    $recordsData = $students->map(function ($s) {
        return [
            'id'          => $s->id,
            ...
            'grades_url'  => route('admin.students.grades.edit', $s),
        ];
    })->values();
@endphp

<script>
    window.recordsData = {!! $recordsData->toJson() !!};
</script>

<div x-data="{
    students: window.recordsData,
    searchQuery: '',
    selectedGrades: [],
    currentPage: 1,
    perPage: 10,
    ...
    <div class="max-w-6xl mx-auto px-6 sm:px-10 lg:px-8 py-4 sm:py-6 lg:py-8">
        ...
        <div class="anim-fade bg-gradient-to-br from-green-600 to-green-800 rounded-2xl shadow-lg px-4 sm:px-8 py-8 relative overflow-hidden">
            ...
            <div class="relative">
                ...
                <p class="text-green-100 text-sm mt-1">All approved and enrolled students</p>
            </div>
                    ...
                    <template x-for="student in pagedStudents" :key="student.id">
                        <tr class="hover:bg-green-50/40 transition-colors duration-150">
                            ...
                            <td class="px-4 sm:px-8 py-3 text-right">
                                <a :href="student.grades_url"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-green-600 hover:bg-green-700 text-white text-xs font-medium transition">
